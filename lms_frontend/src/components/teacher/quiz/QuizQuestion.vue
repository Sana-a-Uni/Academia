<template>
  <div class="main-card">
    <div class="select-button">
      <button class="question-type" @click="goToAddQuestion">
        ADD QUESTION
      </button>
      <button class="question-type" @click="goToReuseQuestion">
        REUSE QUESTION
      </button>
    </div>
    <div class="scrollable-card">
      <div class="question" v-for="(question, index) in questions" :key="index">
        <div class="question-grade">
          ...
          <input type="text" placeholder="Enter the Grade" class="grade" />
        </div>
...
const emit = defineEmits(["go-back", "settings"]);

const questions = ref([
  {
    content: "Here is the content of question 1",
    options: ["Option 1", "Option 2", "Option 3"],
  },
  {
    content: "Here is the content of question 2",
    options: ["Option 1", "Option 2", "Option 3"],
  },
  {
    content: "Here is the content of question 3",
    options: ["Option 1", "Option 2", "Option 3"],
  },
]);
...
const nextPage = () => {
  // Emit event to parent to change the view to Settings
  emit("settings");
};
...
.scrollable-card {
  ...
  height: calc(
    78vh - 80px
  ); /* اجعل البطاقة تشغل ارتفاع الشاشة بالكامل ناقص الأزرار */
  overflow-y: auto;
  border-radius: 10px;
}
