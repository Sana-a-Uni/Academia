...
class Session(Document):
	...
	if TYPE_CHECKING:
		...
		from academia.councils.doctype.session_topic_assignment.session_topic_assignment import (
	...
	def detect_assignments_changes(self):
		...
		old_assignments = {row.name: row for row in old_session.assignments} if old_session else {}
		...
		deleted_assignments = [row for row in old_session.assignments if row.name not in current_assignments]
	...
	def validate_assignments(self):
		for row in self.assignments:
			...
			if not (
				...
				and not assignment.parent_assignment
			):
