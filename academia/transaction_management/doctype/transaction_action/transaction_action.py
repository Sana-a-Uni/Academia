...
class TransactionAction(Document):
    ...
    def on_submit(self):
        self.action_date = frappe.utils.format_datetime(
            self.creation, format_string="dd MMM yyyy, HH:mm:ss"
        )
...
def get_recipients(transaction_name):
    actions = frappe.get_all(
        "Transaction Action",
        filters={"transaction": transaction_name, "docstatus": 1},
        fields=["name"],
    )

    recipients_parent = [a.name for a in actions]
    recipients_parent.append(
        transaction_name
    )  # Add transaction_name to recipients_parent list
    ...
    # TODO: add also start_with user to recipient_emails to avoid recursion in tracking action
