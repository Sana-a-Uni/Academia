...
def get_signatories(doc):
	...
	if doc.start_with:
		start_with = frappe.get_doc("Employee", doc.start_with, fields=["employee_name", "designation"])

		signatories_employee.append(
			{"name": start_with.employee_name, "designation": start_with.designation, "official": False}
		)
		...
		if doc.transaction_scope == "Among Companies":
			...
			signatories_employee.append({"name": name, "designation": designation, "official": True})
...
def get_transaction_category_recipients(transaction_category):
	...
	# Fetch path for the selected transaction category
	transaction_category_path = frappe.get_all(
		"Transaction Path",
		filters={"parent": transaction_category},
		fields=["step", "designation", "print_paper", "has_sign", "is_received"],
		order_by="step ASC",
	)
...
def create_new_transaction_action(user_id, transaction_name, type, details):
	...
	if transaction_doc.through_route:
		...
		company_head = frappe.get_doc(
			"Transaction Company Head", {"company": transaction_doc.start_with_company}
		)
			...
			if action.owner == recipient.recipient_email and action.auto_redirect == 0:
				...
				if action.type == "Redirected":
					...
					redirected_recipients = [
						r for r in redirected_recipients if r.recipient_email not in processed_recipients
					]
		...
		if category_doc.template:
			...
			if linked_fields:
				...
				for item in field_values:
					...
					jinja_values.append({"docfield_title": item["docfield_title"], "value": jinja_value})
...
def get_reports_hierarchy_emp(employee_name):
	...
	while reports_to:
		...
		reports_employee.append(
			{"name": employee.employee_name, "designation": employee.designation, "official": False}
		)
		reports_to = employee.reports_to
...
@frappe.whitelist()
def search_in_actions_for_print_paper_user(
	transacion_name, user, from_company, from_department, from_designation
):
	actions = frappe.get_all(
		"Transaction Action",
		filters={
			"transaction": transacion_name,
			"type": "Redirected",
